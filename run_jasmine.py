...
class JasmineSwitchBetweenCodeAndSpec(sublime_plugin.TextCommand):
  ...
  class JavascriptFile(BaseFile):
    def possible_alternate_files(self): return [self.file_name.replace(".js", "Spec.js")]

  class JasmineFile(BaseFile):
    def possible_alternate_files(self): return [self.file_name.replace("Spec.js", ".js")]

  def current_file(self):
    file_name = self.view.file_name()
    if re.search('\w+\Spec.js', file_name):
